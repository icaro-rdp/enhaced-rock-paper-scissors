...
String check_move(int thumb, int forefinger, int middlefinger, int ringfinger){
  if(thumb<200){
    if(forefinger < 200 & middlefinger < 200){
      return "rock";
    } else {
      return "scissors";  
    }
...
String* checkFingers(){
  ...
  int ringfinger_b = analogRead(A8);
  int littlefinger_b = analogRead(A9);
